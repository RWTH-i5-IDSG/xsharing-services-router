 ...
 * lastLeg    = false
 *
 * Route : START --(walk)-> STATION --(bike)-> STATION --(walk)--> DESTINATION
 *
 * @author Sevket Goekay <goekay@dbis.rwth-aachen.de>
 * @since 24.05.2016
 */
public class WithoutReturnAndNoLastLegStrategy implements MinimalRouteStrategy {

    private final ModeStrategy bikeModeStrategy;

    public WithoutReturnAndNoLastLegStrategy(StrategyDependencyContext ctx) {
        this.bikeModeStrategy = new BikeModeStrategy(ctx, new DefaultBikeInavailabilityStrategy(DurationCheck.ONLY_LEG));
    }
    ...
    public RouteContext findRoute(SingleMinimalRequest request,
        ...
        if (queryTypes.contains(SharingStationType.Bike)) {
            return bikeModeStrategy.findRoute(request, fromPoint, toPoint);
        } else {
